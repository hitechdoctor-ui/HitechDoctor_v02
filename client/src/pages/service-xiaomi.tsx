...
const COMMON_REPAIRS = [
  { icon: Monitor, label: "Αλλαγή Οθόνης",    desc: "AMOLED / LCD γνήσια ή premium ποιότητας" },
...
const jsonLd = {
  ...
  "@type": "Service",
  "name": "Επισκευή Xiaomi — Redmi, Poco & Xiaomi Series",
  "provider": {
    "@type": "LocalBusiness",
    "name": "HiTech Doctor",
  ...
  "description": "Επισκευή Xiaomi, Redmi Note, Redmi και Poco στην Αθήνα. Αλλαγή οθόνης, μπαταρίας, θύρας USB-C. Γνήσια ανταλλακτικά, γραπτή εγγύηση, αποτέλεσμα από 30 λεπτά.",
  "serviceType": "Επισκευή Xiaomi",
  "areaServed": "Αθήνα",
  "hasOfferCatalog": {
    "@type": "OfferCatalog",
    "name": "Επισκευές Xiaomi / Redmi / Poco",
    "itemListElement": XIAOMI_SERIES.flatMap((s) =>
      s.models.map((m) => ({
        "@type": "Offer",
...
export default function ServiceXiaomi() {
  const [activeSeries, setActiveSeries] = useState<string>("all");

  const visibleSeries =
    activeSeries === "all"
      ? XIAOMI_SERIES
      : XIAOMI_SERIES.filter((s) => s.id === activeSeries);

  return (
    <div className="min-h-screen bg-background circuit-bg">
      <Seo
        title="Επισκευή Xiaomi — Redmi Note, Redmi, Poco & Xiaomi Series | HiTech Doctor Αθήνα"
        description="Επισκευή Xiaomi, Redmi Note, Redmi και Poco στην Αθήνα. Αλλαγή οθόνης, μπαταρίας, θύρας USB-C. Γνήσια ανταλλακτικά, εγγύηση, αποτέλεσμα 30 λεπτά."
        url="https://hitechdoctor.com/services/episkeui-xiaomi"
      />
      <Helmet>
        <script type="application/ld+json">{JSON.stringify(jsonLd)}</script>
        <meta name="keywords" content="επισκευή Xiaomi, επισκευή Redmi, επισκευή Poco, αλλαγή οθόνης Xiaomi, αλλαγή μπαταρίας Redmi, επισκευή Xiaomi Αθήνα, service Redmi Note" />
        <link rel="canonical" href="https://hitechdoctor.com/services/episkeui-xiaomi" />
      </Helmet>
      ...
      <main>
        ...
        <div className="container mx-auto px-4 pt-4 pb-0">
          <nav className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap" aria-label="Breadcrumb">
            ...
            <ChevronRight className="w-3 h-3" />
            <span className="text-primary font-medium">Επισκευή Xiaomi</span>
          </nav>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-2xl bg-orange-500/15 border border-orange-500/25 flex items-center justify-center">
                  <SiXiaomi className="w-6 h-6 text-orange-400" />
                </div>
                <div>
                  ...
                  <h1 className="text-2xl sm:text-3xl font-display font-extrabold text-foreground leading-tight">
                    Επισκευή Xiaomi / Redmi / Poco
                  </h1>
                </div>
              </div>
              <p className="text-muted-foreground text-sm leading-relaxed max-w-xl mb-5">
                Εξειδικευμένη επισκευή <strong className="text-foreground">Xiaomi, Redmi Note, Redmi και Poco</strong> στην Αθήνα.
                Αλλαγή οθόνης AMOLED, μπαταρίας, θύρας USB-C και επισκευή πλακέτας.
            ...
            <div className="w-full lg:w-72 shrink-0 grid grid-cols-2 gap-2.5">
              {COMMON_REPAIRS.map((r) => (
                <div key={r.label} className="p-3 rounded-xl border border-white/10 bg-card hover:border-orange-500/20 transition-colors">
                  <r.icon className="w-5 h-5 text-orange-400 mb-2" />
            ...
            <div key={series.id} className="mb-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {series.models.map((model) => (
                  <Link key={model.slug} href={`/episkevi-xiaomi/${model.slug}`}>
                    <article
        ...
        <section className="border-t border-white/8 bg-card/40">
          <div className="container mx-auto px-4 py-12 max-w-6xl">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {[
                { icon: Shield,       title: "Γνήσια Ανταλλακτικά", desc: "Χρησιμοποιούμε γνήσια ή premium ανταλλακτικά Α+ ποιότητας για κάθε μοντέλο." },
