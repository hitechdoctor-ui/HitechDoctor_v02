...
const COMMON_REPAIRS = [
  ...
  { icon: Camera,  label: "Επισκευή Κάμερας",   desc: "Μπροστά / πίσω / Hasselblad οπτική" },
...
const jsonLd = {
  ...
  "@type": "Service",
  "name": "Επισκευή OnePlus — Flagship & Nord Series",
  "provider": {
    "@type": "LocalBusiness",
    "name": "HiTech Doctor",
  ...
  "description": "Επισκευή OnePlus στην Αθήνα. Flagship (OnePlus 13, 12, 11, 10 Pro) και Nord Series. Αλλαγή οθόνης AMOLED, μπαταρίας, θύρας USB-C. Γνήσια ανταλλακτικά, γραπτή εγγύηση, αποτέλεσμα 30 λεπτά.",
  "serviceType": "Επισκευή OnePlus",
  "areaServed": "Αθήνα",
  "hasOfferCatalog": {
    "@type": "OfferCatalog",
    "name": "Επισκευές OnePlus",
    "itemListElement": ONEPLUS_SERIES.flatMap((s) =>
      s.models.map((m) => ({
        "@type": "Offer",
...
export default function ServiceOnePlus() {
  const [activeSeries, setActiveSeries] = useState<string>("all");

  const visibleSeries =
    activeSeries === "all"
      ? ONEPLUS_SERIES
      : ONEPLUS_SERIES.filter((s) => s.id === activeSeries);

  return (
    <div className="min-h-screen bg-background circuit-bg">
      <Seo
        title="Επισκευή OnePlus — Flagship & Nord Series | HiTech Doctor Αθήνα"
        description="Επισκευή OnePlus 13, 12, 11, 10 Pro, Nord 4 στην Αθήνα. Αλλαγή οθόνης AMOLED, μπαταρίας, USB-C. Γνήσια ανταλλακτικά, εγγύηση, αποτέλεσμα 30 λεπτά."
        url="https://hitechdoctor.com/services/episkeui-oneplus"
      />
      <Helmet>
        <script type="application/ld+json">{JSON.stringify(jsonLd)}</script>
        <meta name="keywords" content="επισκευή OnePlus, αλλαγή οθόνης OnePlus, αλλαγή μπαταρίας OnePlus, επισκευή OnePlus Αθήνα, service OnePlus Nord" />
        <link rel="canonical" href="https://hitechdoctor.com/services/episkeui-oneplus" />
      </Helmet>
      ...
      <main>
        ...
        <div className="container mx-auto px-4 pt-4 pb-0">
          <nav className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap" aria-label="Breadcrumb">
            ...
            <ChevronRight className="w-3 h-3" />
            <span className="text-primary font-medium">Επισκευή OnePlus</span>
          </nav>
        </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-2xl bg-red-500/15 border border-red-500/25 flex items-center justify-center">
                  <SiOneplus className="w-6 h-6 text-red-400" />
                </div>
                <div>
              ...
              <p className="text-muted-foreground text-sm leading-relaxed max-w-xl mb-5">
                Εξειδικευμένη επισκευή <strong className="text-foreground">OnePlus Flagship Series και Nord Series</strong> στην Αθήνα.
            ...
            <div key={series.id} className="mb-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {series.models.map((model) => (
                  <Link key={model.slug} href={`/episkevi-oneplus/${model.slug}`}>
                    <article
        ...
        <section className="border-t border-white/8 bg-card/40">
          <div className="container mx-auto px-4 py-12 max-w-6xl">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {[
                { icon: Shield,       title: "Γνήσια Ανταλλακτικά", desc: "Χρησιμοποιούμε γνήσια ή premium ανταλλακτικά Α+ ποιότητας για κάθε μοντέλο OnePlus." },
