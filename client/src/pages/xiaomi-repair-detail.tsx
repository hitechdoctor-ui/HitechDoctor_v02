...
export default function XiaomiRepairDetail() {
  const [, params] = useRoute("/episkevi-xiaomi/:slug");
  const modelSlug = params?.slug ?? "";
  const baseModel = findXiaomiBySlug(modelSlug);
  const priceMap = useRepairPriceOverrideMap();
  const model = useMemo(
    () => (baseModel ? mergeAndroidRepairModel(baseModel, "xiaomi", priceMap) : null),
    [baseModel, priceMap]
  );
  const series = findXiaomiSeriesForModel(modelSlug);
  ...
  const canonicalUrl = `https://hitechdoctor.com/episkevi-xiaomi/${model.slug}`;
  ...
  const breadcrumbLd = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": "Επισκευή Xiaomi", "item": "https://hitechdoctor.com/services/episkeui-xiaomi" },
      { "@type": "ListItem", "position": 4, "name": model.name, "item": canonicalUrl },
    ],
  };

  const otherModels = XIAOMI_SERIES.flatMap((s) => s.models).filter((m) => m.slug !== model.slug).slice(0, 10);
    ...
    <div className="min-h-screen bg-background circuit-bg">
      ...
      <main className="container mx-auto max-w-6xl px-4 pt-6 pb-[calc(7.5rem+env(safe-area-inset-bottom))] md:pb-20">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-muted-foreground mb-6 flex-wrap">
          ...
          <ChevronRight className="w-3 h-3 opacity-40" />
          <Link href="/services/episkeui-xiaomi" className="hover:text-primary transition-colors">Επισκευή Xiaomi</Link>
          <ChevronRight className="w-3 h-3 opacity-40" />
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-8">
          <div className="lg:col-span-2 space-y-10">
            ...
            {/* Other models */}
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground/70 mb-3">Άλλα Μοντέλα Xiaomi / Redmi / Poco</p>
              <div className="flex flex-wrap gap-2">
                {otherModels.map((m) => (
                  <Link key={m.slug} href={`/episkevi-xiaomi/${m.slug}`}>
